...
function getAqiFromPM25(pm25) {                                                                                           if (pm25 < lowerBoundsPM25[0] || pm25 > upperBoundsPM25[upperBoundsPM25.length - 1]) {
    throw new Error(`PM2.5 must be between ${lowerBoundsPM25[0]} and ${upperBoundsPM25[upperBoundsPM25.length - 1]}, but was ${pm25}`);
  }
                                                                                                                          let idx = 0
  while (pm25 > upperBoundsPM25[idx]) {                                                                                     idx += 1                                                                                                              }
                                                                                                                          let slope = (upperBoundsAQI[idx] - lowerBoundsAQI[idx]) / (upperBoundsPM25[idx] - lowerBoundsPM25[idx]);                 let aqi = slope * (pm25 - lowerBoundsPM25[idx]) + lowerBoundsAQI[idx];
